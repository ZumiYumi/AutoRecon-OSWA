...
from autorecon_oswa.config import config, configurable_keys, configurable_boolean_keys
from autorecon_oswa.io import slugify, e, fformat, cprint, debug, info, warn, error, fail, CommandStreamReader
...
def cancel_all_tasks(sig, frame):
	...
	for target in autorecon_oswa.scanning_targets:
...
async def get_semaphore(autorecon_oswa):
	semaphore = autorecon_oswa.service_scan_semaphore
	while True:
		# If service scan semaphore is locked, see if we can use port scan semaphore.
		if semaphore.locked():
			if semaphore != autorecon_oswa.port_scan_semaphore: # This will be true unless user sets max_scans == max_port_scans
				...
				if not autorecon_oswa.pending_targets and (config['max_port_scans'] - port_scan_task_count) >= 1: # If no more targets, and we have room, use port scan semaphore.
					if autorecon_oswa.port_scan_semaphore.locked():
						await asyncio.sleep(1)
						continue
					semaphore = autorecon_oswa.port_scan_semaphore
					break
				else: # Do some math to see if we can use the port scan semaphore.
					if (config['max_port_scans'] - (port_scan_task_count + (len(autorecon_oswa.pending_targets) * config['port_scan_plugin_count']))) >= 1:
						if autorecon_oswa.port_scan_semaphore.locked():
							await asyncio.sleep(1)
							continue
						semaphore = autorecon_oswa.port_scan_semaphore
...
async def port_scan(plugin, target):
	...
	async with target.autorecon_oswa.port_scan_semaphore:
...
async def service_scan(plugin, service):
	semaphore = service.target.autorecon_oswa.service_scan_semaphore

	if not config['force_services']:
		semaphore = await get_semaphore(service.target.autorecon_oswa)
	...
	while plugin_pending:
		...
		if plugin.max_global_instances and plugin.max_global_instances > 0:
			async with service.target.autorecon_oswa.lock:
				# Count currently running plugin instances.
				for target in service.target.autorecon_oswa.scanning_targets:
		...
		async with semaphore:
			...
			nmap_extra = service.target.autorecon_oswa.args.nmap
			if service.target.autorecon_oswa.args.nmap_append:
				nmap_extra += ' ' + service.target.autorecon_oswa.args.nmap_append
...
async def generate_report(plugin, targets):
	...
	if not config['force_services']:
		semaphore = await get_semaphore(autorecon_oswa)
...
async def scan_target(target):
	...
	if config['force_services']:
		...
		if services:
			...
			heartbeat.cancel()
			autorecon_oswa.errors = True
			return
	else:
		for plugin in target.autorecon_oswa.plugin_types['port']:
			...
			if config['port_scans'] and plugin.slug in config['port_scans']:
				...
				matching_tags = False
				for tag_group in target.autorecon_oswa.tags:
					if set(tag_group).issubset(plugin_tag_set):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in target.autorecon_oswa.excluded_tags:
					if set(tag_group).issubset(plugin_tag_set):
						excluded_tags = True
						break
	...
	async with autorecon_oswa.lock:
		autorecon_oswa.scanning_targets.append(target)
	...
	while pending:
		...
		for service in services:
			...
			nmap_extra = target.autorecon_oswa.args.nmap
			if target.autorecon_oswa.args.nmap_append:
				nmap_extra += ' ' + target.autorecon_oswa.args.nmap_append
			...
			for plugin in target.autorecon_oswa.plugin_types['service']:
			...
			if not service_match:
				...
				if service.name not in config['service_exceptions'] and service.full_tag() not in target.autorecon_oswa.missing_services:
					target.autorecon_oswa.missing_services.append(service.full_tag())

	for plugin in target.autorecon_oswa.plugin_types['report']:
	...
	async with autorecon_oswa.lock:
		autorecon_oswa.completed_targets.append(target)
		autorecon_oswa.scanning_targets.remove(target)
...
async def run():
	...
	autorecon_oswa.argparse = parser
	...
	for plugins_dir in plugins_dirs:
		for plugin_file in sorted(os.listdir(plugins_dir)):
			if not plugin_file.startswith('_') and plugin_file.endswith('.py'):
				...
				try:
					spec = importlib.util.spec_from_file_location("autorecon_oswa." + filename[:-3], os.path.join(dirname, filename))
					plugin = importlib.util.module_from_spec(spec)
					spec.loader.exec_module(plugin)

					clsmembers = inspect.getmembers(plugin, predicate=inspect.isclass)
					for (_, c) in clsmembers:
						if c.__module__ in ['autorecon_oswa.plugins', 'autorecon_oswa.targets']:
							continue
						...
						if issubclass(c, PortScan) or issubclass(c, ServiceScan) or issubclass(c, Report):
							autorecon_oswa.register(c(), filename)
						else:
	...
	for plugin in autorecon_oswa.plugins.values():
		if plugin.slug in autorecon_oswa.taglist:
			unknown_help()
	...
	if len(autorecon_oswa.plugin_types['port']) == 0:
		unknown_help()
		fail('Error: There are no valid PortScan plugins in the plugins directory "' + config['plugins_dir'] + '".')

	# Sort plugins by priority.
	autorecon_oswa.plugin_types['port'].sort(key=lambda x: x.priority)
	autorecon_oswa.plugin_types['service'].sort(key=lambda x: x.priority)
	autorecon_oswa.plugin_types['report'].sort(key=lambda x: x.priority)
						...
						if 'pattern' in pattern:
							try:
								...
								if 'description' in pattern:
									autorecon_oswa.patterns.append(Pattern(compiled, description=pattern['description']))
								else:
									autorecon_oswa.patterns.append(Pattern(compiled))
							except re.error:
								unknown_help()
		...
		if key == 'global' and isinstance(val, dict): # Process global plugin options.
			for gkey, gval in config_toml['global'].items():
				if isinstance(gval, bool):
					for action in autorecon_oswa.argparse._actions:
						if action.dest == 'global.' + slugify(gkey).replace('-', '_'):
							if action.const is True:
								action.__setattr__('default', gval)
							break
				else:
					if autorecon_oswa.argparse.get_default('global.' + slugify(gkey).replace('-', '_')):
						autorecon_oswa.argparse.set_defaults(**{'global.' + slugify(gkey).replace('-', '_'): gval})
		elif isinstance(val, dict): # Process potential plugin arguments.
			for pkey, pval in config_toml[key].items():
				if autorecon_oswa.argparse.get_default(slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_')) is not None:
					for action in autorecon_oswa.argparse._actions:
						if action.dest == slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'):
							if action.const and pval != action.const:
								...
								errors = True
							break
					autorecon_oswa.argparse.set_defaults(**{slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'): pval})
		else: # Process potential other options.
			key = key.replace('-', '_')
			if key in configurable_keys:
				other_options.append(key)
				config[key] = val
				autorecon_oswa.argparse.set_defaults(**{key: val})
	...
	if config['max_plugin_target_instances']:
		for plugin_instance in config['max_plugin_target_instances']:
			...
			if len(plugin_instance) == 2:
				if plugin_instance[0] not in autorecon_oswa.plugins:
	...
	for slug, plugin in autorecon_oswa.plugins.items():
		...
		for member_name, _ in inspect.getmembers(plugin, predicate=inspect.ismethod):
			if member_name == 'check':
				if plugin.check() == False:
					autorecon_oswa.plugins.pop(slug)
					continue
				continue
	...
	if not errors:
		if config['force_services']:
			autorecon_oswa.service_scan_semaphore = asyncio.Semaphore(config['max_scans'])
		else:
			autorecon_oswa.port_scan_semaphore = asyncio.Semaphore(config['max_port_scans'])
			...
			if config['max_scans'] == config['max_port_scans']:
				autorecon_oswa.service_scan_semaphore = autorecon_oswa.port_scan_semaphore
			else:
				autorecon_oswa.service_scan_semaphore = asyncio.Semaphore(config['max_scans'] - config['max_port_scans'])
	...
	[autorecon_oswa.tags.append(t) for t in tags if t not in autorecon_oswa.tags]
	...
	if args.exclude_tags != '':
		...
		# Remove duplicate lists from list.
		[autorecon_oswa.excluded_tags.append(t) for t in excluded_tags if t not in autorecon_oswa.excluded_tags]
	...
	for target in raw_targets:
		try:
			...
			found = False
			for t in autorecon_oswa.pending_targets:
				if t.address == ip_str:
					found = True
					break
			...
			if isinstance(ip, ipaddress.IPv4Address):
				autorecon_oswa.pending_targets.append(Target(ip_str, ip_str, 'IPv4', 'ip', autorecon_oswa))
			elif isinstance(ip, ipaddress.IPv6Address):
				autorecon_oswa.pending_targets.append(Target(ip_str, ip_str, 'IPv6', 'ip', autorecon_oswa))
			else:
			...
			try:
				...
				try:
					...
					autorecon_oswa.pending_targets.append(Target(target, ip, 'IPv4', 'hostname', autorecon_oswa))
					...
					try:
						...
						autorecon_oswa.pending_targets.append(Target(target, ip, 'IPv6', 'hostname', autorecon_oswa))
					except socket.gaierror:
	...
	if len(autorecon_oswa.pending_targets) == 0:
		error('You must specify at least one target to scan!')
		errors = True

	if config['single_target'] and len(autorecon_oswa.pending_targets) != 1:
		error('You cannot provide more than one target when scanning in single-target mode.')
		errors = True

	if not args.disable_sanity_checks and len(autorecon_oswa.pending_targets) > 256:
		error('A total of ' + str(len(autorecon_oswa.pending_targets)) + ' targets would be scanned. If this is correct, re-run with the --disable-sanity-checks option to suppress this check.')
		errors = True

	if not config['force_services']:
		port_scan_plugin_count = 0
		for plugin in autorecon_oswa.plugin_types['port']:
			if config['port_scans'] and plugin.slug in config['port_scans']:
				matching_tags = True
				excluded_tags = False
			else:
				matching_tags = False
				for tag_group in autorecon_oswa.tags:
					if set(tag_group).issubset(set(plugin.tags)):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in autorecon_oswa.excluded_tags:
					if set(tag_group).issubset(set(plugin.tags)):
						excluded_tags = True
						break
	...
	pending = []
	i = 0
	while autorecon_oswa.pending_targets:
		pending.append(asyncio.create_task(scan_target(autorecon_oswa.pending_targets.pop(0))))
		i+=1
	...
	while pending:
		...
		# If something failed in scan_target, autorecon_oswa.errors will be true.
		if autorecon_oswa.errors:
		...
		for task in done:
			if autorecon_oswa.pending_targets:
				pending.add(asyncio.create_task(scan_target(autorecon_oswa.pending_targets.pop(0))))
			if task in pending:
				pending.remove(task)

		port_scan_task_count = 0
		for targ in autorecon_oswa.scanning_targets:
	...
	if len(autorecon_oswa.completed_targets) > 1:
		for plugin in autorecon_oswa.plugin_types['report']:
			if config['reports'] and plugin.slug in config['reports']:
				matching_tags = True
				excluded_tags = False
			else:
			...
			if matching_tags and not excluded_tags:
				pending.add(asyncio.create_task(generate_report(plugin, autorecon_oswa.completed_targets)))
	...
	if autorecon_oswa.missing_services:
		warn('{byellow}AutoRecon identified the following services, but could not match them to any plugins based on the service name. Please report these to Tib3rius: ' + ', '.join(autorecon_oswa.missing_services) + '{rst}')
